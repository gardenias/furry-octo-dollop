...
import com.g.fod.endpoints.binance.spec.impl.BinanceAccountRestSpecImpl;
...
class BinanceRestSpecTest {
  ...
  void testMarket() {
    ...
    req.setLimit(10);
    Flux<OldTradeLookupResp> oldTradeLookupRespMono = marketRestSpec.oldTradeLookupSpot(req);
  ...
  void testRecentTradesList() throws InterruptedException {
    ...
    req.setSymbol("BTCUSDT");
    req.setLimit(10);
    Flux<RecentTradesListResp> oldTradeLookupRespMono = marketRestSpec.recentTradesListSpot(req);
  ...
  void testRecentTradesList1() throws InterruptedException {
    ...
    Flux<RecentTradesListResp> flux = webClient.get().uri("/api/v3/trades", req).retrieve()
      .bodyToFlux(RecentTradesListResp.class);
